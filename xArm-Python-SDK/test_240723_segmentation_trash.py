...
class RobotMain(object):
    ...
    def trash_mode(self):
        ...
        if center_x_mm < -300 and center_y_mm > -130 and center_y_mm < 100:
            ...
            code = self._arm.set_position(*[center_x_mm+110, center_y_mm, 150.6, 180, -77.1, -180], speed=self._tcp_speed,
                                            mvacc=self._tcp_acc, radius=0.0, wait=True)
            if not self._check_code(code, 'set_position'): return
        ...
        if center_x_mm > -300 and center_x_mm < -100 and center_y_mm > -130 and center_y_mm < 110:
            ...
            code = self._arm.set_servo_angle(angle=[180, 61, 96.3, 180, -96.2, 180], speed=self._angle_speed,
                                            mvacc=self._angle_acc, wait=True, radius=0.0)
            if not self._check_code(code, 'set_servo_angle'): return
            ...
            code = self._arm.set_servo_angle(angle=[180, 76.4, 126.7, 180, -112.6, 180], speed=self._angle_speed,
                                            mvacc=self._angle_acc, wait=True, radius=0.0)
            if not self._check_code(code, 'set_servo_angle'): return
            ...
            code = self._arm.set_position(x=center_x_mm+312.4, radius=-1, speed=self._tcp_speed, mvacc=self._tcp_acc, relative=True,
                                            wait=True)
            if not self._check_code(code, 'set_position'): return
            ...
            code = self._arm.set_position(z=72, radius=-1, speed=self._tcp_speed, mvacc=self._tcp_acc, relative=True,
                                            wait=True)
            if not self._check_code(code, 'set_position'): return

            # -- 쓰레기 버리러 가기 --
            code = self._arm.set_servo_angle(angle=[173.9, 19, 37.8, 147.3, -92, 180], speed=self._angle_speed,
                                            mvacc=self._angle_acc, wait=True, radius=0.0)
            if not self._check_code(code, 'set_servo_angle'): return
            
            code = self._arm.set_servo_angle(angle=[126.7, -8.4, 5.4, 100, -85.4, 180], speed=self._angle_speed,
                                            mvacc=self._angle_acc, wait=True, radius=0.0)
            if not self._check_code(code, 'set_servo_angle'): return

            code = self._arm.set_servo_angle(angle=[77.2, -8.3, 6.9, 41.3, -70.9, 0], speed=self._angle_speed,
                                            mvacc=self._angle_acc, wait=True, radius=0.0)
            if not self._check_code(code, 'set_servo_angle'): return
            ...
            code = self._arm.set_position(z=42, radius=-1, speed=self._tcp_speed, mvacc=self._tcp_acc, relative=True,
                                            wait=True)
            if not self._check_code(code, 'set_position'): return
